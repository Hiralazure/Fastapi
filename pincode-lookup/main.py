...
app=FastAPI(title="Pincode lookup API",description="Auto fill city and state from India pincode during checkout")
...
def bulk_lookup(request:BulkRequest):
    ...
    for code in request.pincodes:
        if code in pincode_db:
            ...
            missing.append(code)
    return BulkResponse(found= len(results),not_found=len(missing),results=results, missing=missing)